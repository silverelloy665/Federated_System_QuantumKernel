"""
Phase A: Discovery & Branch Routing (Optimized Memory & Chunking)
Locates datasets, inspects schemas, demultiplexes sub-tables, and classifies into Network vs Physical branches.
"""
...
def read_excel_fast(file_obj, sheet_name=None, max_rows=25000) -> pd.DataFrame:
...
class DiscoveryRouter:
    ...
    def profile_dataframe(self, name: str, source_file: str, df: pd.DataFrame, branch: str) -> Dict[str, Any]:
        """Profiles dataset characteristics for dataset_inventory.csv."""
    ...
    def run_discovery(self) -> Tuple[List[Dict[str, Any]], Dict[str, List[pd.DataFrame]]]:
        """
        Executes Phase A: Discovers, routes, profiles, and generates inventory.
        """
        print("\n========================================================", flush=True)
        ...
        if "uavids" in self.resolved_paths:
            ...
            branch = self.classify_schema(list(df.columns))
            self.profile_dataframe("UAVIDS-2025", path.name, df, branch)
            df['_source_dataset'] = "UAVIDS-2025"
            routed_data[branch].append(df)
            print(f"    -> Routed {df.shape[0]:,} rows to {branch} Branch.", flush=True)
        ...
        if "tits" in self.resolved_paths:
            ...
            for seg_name, df, branch in segments:
                self.profile_dataframe(seg_name, path.name, df, branch)
                df['_source_dataset'] = seg_name
                routed_data[branch].append(df)
                print(f"    -> Routed [{seg_name}] {df.shape[0]:,} rows to {branch} Branch.", flush=True)

        # 3. UAV-NDD CSV.zip
        if "uav_ndd" in self.resolved_paths:
            path = self.resolved_paths["uav_ndd"]
            print(f"[*] Extracting streaming samples from {path.name}...", flush=True)
            with zipfile.ZipFile(path, 'r') as z:
                # UAV-Case1-Label.csv (Sample up to 60,000 rows across chunks for memory efficiency)
                if "UAV-NDD CSV/UAV-Case1-Label.csv" in z.namelist():
                    with z.open("UAV-NDD CSV/UAV-Case1-Label.csv") as f:
                        df_c1 = pd.read_csv(f, encoding='latin1', on_bad_lines='skip', nrows=60000, low_memory=False)
                        branch = self.classify_schema(list(df_c1.columns))
                        self.profile_dataframe("UAV-NDD_Case1", path.name, df_c1, branch)
                        df_c1['_source_dataset'] = "UAV-NDD_Case1"
                        routed_data[branch].append(df_c1)
                        print(f"    -> Routed UAV-Case1 sample {df_c1.shape[0]:,} rows to {branch} Branch.", flush=True)
                ...
                if "UAV-NDD CSV/Access Point Case2 Label.xlsx" in z.namelist():
                    with z.open("UAV-NDD CSV/Access Point Case2 Label.xlsx") as f:
                        df_ap = read_excel_fast(f, max_rows=15000)
                        branch = self.classify_schema(list(df_ap.columns))
                        self.profile_dataframe("UAV-NDD_AccessPoint_Case2", path.name, df_ap, branch)
                        df_ap['_source_dataset'] = "UAV-NDD_AccessPoint_Case2"
                        routed_data[branch].append(df_ap)
                        print(f"    -> Routed Access Point Case2 {df_ap.shape[0]:,} rows to {branch} Branch.", flush=True)
                ...
                if "UAV-NDD CSV/GSC Case3 Label .csv" in z.namelist():
                    with z.open("UAV-NDD CSV/GSC Case3 Label .csv") as f:
                        df_gsc = read_excel_fast(f, sheet_name="Dataset-UAV-GCS", max_rows=15000)
                        branch = self.classify_schema(list(df_gsc.columns))
                        self.profile_dataframe("UAV-NDD_GSC_Case3", path.name, df_gsc, branch)
                        df_gsc['_source_dataset'] = "UAV-NDD_GSC_Case3"
                        routed_data[branch].append(df_gsc)
                        print(f"    -> Routed GSC Case3 {df_gsc.shape[0]:,} rows to {branch} Branch.", flush=True)

        # 4. CSV.zip (CIC-IoT proxies)
        if "csv_iot" in self.resolved_paths:
            path = self.resolved_paths["csv_iot"]
            print(f"[*] Sampling representative flows from {path.name}...", flush=True)
            with zipfile.ZipFile(path, 'r') as z:
                ...
                for cat_folder, files in list(folder_map.items())[:20]:
                    ...
                    for f_name in files[:1]:
                        with z.open(f_name) as f:
                            chunk = pd.read_csv(f, nrows=1000, encoding_errors='replace', low_memory=False)
                ...
                if sampled_dfs:
                    ...
                    self.profile_dataframe("CIC_IoT_Proxies", path.name, df_iot, branch)
                    df_iot['_source_dataset'] = "CIC_IoT_Proxies"
                    routed_data[branch].append(df_iot)
                    print(f"    -> Routed CIC-IoT proxy flows {df_iot.shape[0]:,} rows to {branch} Branch.", flush=True)
        ...
        if flow_zip:
            print(f"[*] Sampling network flows from {flow_zip.name}...", flush=True)
            with zipfile.ZipFile(flow_zip, 'r') as z:
                ...
                for cf in csv_files[:5]:
                    with z.open(cf) as f:
                        chunk = pd.read_csv(f, nrows=2000, encoding_errors='replace', low_memory=False)
                ...
                if sampled_flows:
                    ...
                    self.profile_dataframe("CICIDS2017_NetworkFlows", flow_zip.name, df_cic, branch)
                    df_cic['_source_dataset'] = "CICIDS2017"
                    routed_data[branch].append(df_cic)
                    print(f"    -> Routed CICIDS2017 flows {df_cic.shape[0]:,} rows to {branch} Branch.", flush=True)
